...
{
	GEMPRules()->SetScoreboardMode(SCOREBOARD_POINTS_STANDARD);
}

void pyEnableTimeBasedScoring()
{
	GEMPRules()->SetScoreboardMode(SCOREBOARD_POINTS_TIME);
}

void pyEnableLevelBasedScoring()
{
...
BOOST_PYTHON_MODULE(GEMPGameRules)
{
	...
	class_<CGERadarResource, boost::noncopyable>("CGERadar", no_init)
		...
		.def("AddRadarOverlay", &CGERadarResource::AddRadarOverlay, ("entity", arg("icon")="", arg("color")=Color()))
		.def("DropRadarOverlay", &CGERadarResource::DropRadarOverlay)
		.def("HasRadarOverlay", &CGERadarResource::HasRadarOverlay)
