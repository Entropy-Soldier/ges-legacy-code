...
void ExplosionCreate(const Vector &center, const QAngle &angles,
    ...
    const EHANDLE *ignoredEntity, Class_T ignoredClass)
{
    ExplosionCreate( center, angles, pOwner, magnitude, INT_MAX, radius, nSpawnFlags, flExplosionForce, 1.0f, pInflictor, iCustomDamageType, ignoredEntity, ignoredClass );
}
#else
// HACKHACK -- create one of these and fake a keyvalue to get the right explosion setup
void ExplosionCreate( const Vector &center, const QAngle &angles, 
